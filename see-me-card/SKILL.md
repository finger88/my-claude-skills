...
9. **出卡**：按 `references/card-spec.md` 写 `card_spec.json`，再运行 `scripts/render_card.js` 生成 HTML 和 PNG。
10. **验卡**：确认 PNG 为 1080x1440，查看成图，若文字挤压或重叠，先改 spec 或 CSS 后重渲染。
11. **写入连续台账**：按 `references/continuity-ledger.md` 追加今日沉积。台账是可修正的工作假设，不是用户长期记忆。
