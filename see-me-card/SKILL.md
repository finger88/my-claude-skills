...
10. **出证据回场卡**：从当日 `evidence.md` 里挑 1 条最能唤回现场的证据，另写一份 `card_spec.json`，输出到 `YYYYMMDD_<主题词>_scene_card`。这张卡少解释、多源头，用来让用户在疲惫时也能回到记下想法的那一刻。
...
- 标题要像把用户带回动作现场，例如“回到那一刻 / 你问书”；避免抽象主题词堆叠。
- `sourceEcho` 放用户原始想法；`mainText` 放最短的触发原文或现场判断；`nodes` 控制在 3 条左右：原句 / 想法 / 现场。
